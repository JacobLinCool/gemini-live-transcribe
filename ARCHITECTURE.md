...
`gemini-live-transcribe` is a single-binary terminal application for live transcription with Gemini Live. It captures one or more local audio sources, streams each source to its own Gemini Live session, and renders the resulting transcripts in a minimal ratatui interface.
...
- `src/paths.rs`: canonical OS-standard paths for config and logs.
