...
- `SourcePane`: per-source view model containing source status, committed turns, current draft, token/context snapshot, and cached layout state.
...
2. `main.rs` routes those chunks into one transcriber supervisor per source.
3. `transcriber.rs` converts Gemini Live server messages into typed events.
4. `main.rs` turns those typed events into `AppEvent` values.
...
- Draft text should remain visually distinct from committed text.
